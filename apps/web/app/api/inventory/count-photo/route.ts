...
export async function POST(request: Request) {
  ...
  if (!session.session.activeOrganizationId)
    return NextResponse.json(
      { error: "Choose a company before counting inventory" },
      { status: 403 },
    );
  ...
  if (!result.ok)
    return NextResponse.json(
      {
        error: "Gemini is temporarily busy after automatic retries. Try again.",
      },
      { status: 503 },
    );
  ...
  try {
    return NextResponse.json(countResultSchema.parse(JSON.parse(text)));
  } catch {
    return NextResponse.json(
